🚀 TaskMaster API: Setup & CI/CD DocumentationThis repository contains a PHP/HTML-based task management application. We use Docker for local development and Jenkins for an automated "Smoke Test" deployment pipeline.1. Project OverviewTechnology Stack: PHP (Apache), HTML, CSS.Infrastructure: Docker & Docker Compose.CI/CD: Jenkins (running as a Docker container) with a multi-stage Jenkinsfile.2. Local Development SetupTo run this project on your local machine (Mac/Ubuntu):Clone the Repository:Bashgit clone https://github.com/ahsanalijee/taskmaster-api.git
cd taskmaster-api
Environment Variables:Bashcp .env.example .env
Start the Application:Bashdocker-compose up -d --build
Access the App: Open http://localhost:8081 in your browser.3. Jenkins Infrastructure SetupWe run Jenkins inside Docker. To allow Jenkins to build our project images, it must "speak" to the host's Docker engine.A. Run Jenkins ContainerBashdocker run -d \
  -p 8085:8080 \
  --name jenkins-server \
